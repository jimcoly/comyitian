...
function autoRun() {
	if(document.body){
		...
		if (document.getElementById("nav") && document.getElementById("chrome-title")) {
			addSwitchButton();
			// search box event
			byId("search-input").addEventListener("click",function(){
				cssClassOps("add", byId("search"), "on", null);
			},false);
			byId("main").addEventListener("click",function(){
				cssClassOps("remove", byId("search"), "on", null);
			},false);
			// egg
			var acc = $("#guser .gb4")[0].innerText;
			if( acc == "amio.cx@gmail.com"){
				document.body.style.fontFamily = "verdana";
			}else if( acc == "myth.mio@gmail.com"){
				document.body.style.fontFamily = "verdana";
				$("#guser .gb4")[0].innerText = acc + " (4Miu!)";
			}
